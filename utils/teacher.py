...
from .data import Dataset, to_tensor, ImagePool
...
class Knowledge(Dataset):
    ...
    def __init__(self,
        ...
        self.cache = cache
        self.topk = topk
        super().__init__(imgpool=imgpool, indexes=indexes)
        # 获取数据增强组件
        self.get_tf(hyp)
    ...
    def train(self):
        ...
        if len(getattr(self, 'knowledge', tuple())):
            LOGGER.info(f'\nKnowledge has been completely emptied')
