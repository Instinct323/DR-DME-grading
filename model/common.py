...
class SPPF(nn.Module):
    ...
    def __init__(self, c1, c2, k=5, n=3, e=0.5):  # equivalent to SPP(k=(5, 9, 13))
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1)
...
class AttnPool2d(nn.Module):
    ...
    def __init__(self, c1, p, nhead=8, drop=0., bias=True):
        ...
        self.mha = MultiheadAttn(c1, nhead=nhead, s=0, drop=drop, qk_norm=False, bias=bias)
...
class MixerLayer(nn.Module):
    ...
    def __init__(self, c1, p, e=.5, drop=0.1):
        super(MixerLayer, self).__init__()
        self.ln1 = nn.LayerNorm(c1)
        self.mlp1 = Mlp(p, e=e, drop=drop, act=QuickGELU)
        self.ln2 = nn.LayerNorm(c1)
        self.mlp2 = Mlp(c1, e=e, drop=drop, act=QuickGELU)
...
class PosEmbedding(nn.Module):
    ...
    def __init__(self, c1, etype='fourier', w=None, h=None, f=10.):
        ...
        if etype == 'simple':
            ...
            self.embedding = None
            self.embedder = FourierFeatures(c1, f=f, seed=0)
        else:
    ...
    def forward(self, x):
        ...
        # fourier feature
            ...
            if self.embedding is None or not equal:
                self.hw = LOCAL.hw
                self.embedding = torch.from_numpy(self.embedder(*reversed(self.hw))
...
class MultiheadAttn(nn.Module):
    ...
    def qkv_proj(self, query, key=None):
        ...
        B, L, C = key.shape
        multi_head = (B, -1, self.nhead, self.chead) if self.nhead != 1 else None
...
class GroupingLayer(nn.Module):
    ...
    def forward(self, x):
        # Transformer Encoder
        group_token = self.group_token.repeat(x.shape[0], 1, 1)
