...
class YamlModel(nn.ModuleList):
    ...
    def init_param(self):
        for m in self.modules():
            ...
            #     c2, g = m.out_channels, m.groups
            #     fan_out = c2 * math.prod(m.kernel_size) / g
    ...
    def torchscript(self, x):
        model = torch.jit.trace(self.simplify(inplace=False), (x,))
    ...
    def parse_architecture(self, in_channels, ch_divisor=4):
        LOGGER.info('\n%3s %17s %2s %9s  %-15s %-30s' % ('', 'from', 'n', 'params', 'module', 'arguments'))
        modules, channels, self.save = [], [in_channels], set()
        for i, (from_, number, module, args) in enumerate(copy.deepcopy(self.cfg['architecture'])):
            kwargs = {}
            # 使用深度增益对 number 进行修改
            if i not in self.cfg['fixed_layers']: number = max([int(number * self.cfg['depth_multiple']), 1])
            # 对 module 进行转换
            ...
            if isinstance(from_, int):
                ...
                assert module is Concat, f'Bad argument for <{module}>: from = {from_}'
                c2 = sum([channels[f] for f in from_])
            # 实例化模块
