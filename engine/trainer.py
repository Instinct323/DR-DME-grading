...
class Trainer:
    ...
    def set_logger(self, m_title, s=' %9s', n=' %9.4g'):
        ''' m_title: 各个度量的名称'''
        # train: 损失
        self._l_title = (s * 5) % ('epoch', 'GPU-mem', 'loss', 'lr', 'error')
        self._l_format = s + n * 3 + s
        # eval: 度量
        assert len(m_title) > 0
        m_title = tuple(map(lambda s: s.replace(' ', '_'), m_title))
        self._m_title = (s * (len(m_title) + 1)) % ('', *m_title)
        self._m_format = s + n * len(m_title)
    ...
    def __call__(self, train_set, eval_set=None):
        start_epoch, model_file = 0, self.project / 'last.pt'
        if model_file.is_file():
        ...
        for epoch in range(start_epoch + 1, self._epochs + 1):
            ...
            for file in dump_files: torch.save(ckpt, self.project / file)
            # 保存结果到 Result
    ...
    def eval(self, epoch, eval_set):
        ...
        LOGGER.info(self._m_format % ('', *metrics))
        # 保存在验证集上表现最好的网络
    ...
    def finish(self, result):
        try:
            ...
            # 添加 fitness, 并绘制曲线图
            result['fitness'] = list(map(self.fitness, result[self._m_title.split()].to_numpy()))
        ...
        if model_file.is_file():
            ...
            if metrics.size > 0:
                self.best_fitness = self.fitness(metrics)
                # 输出最优模型的信息
                LOGGER.info(self._m_title)
                LOGGER.info(self._m_format % ('best', *ckpt['metrics']) + '\n')
    ...
    def _run_one_epoch(self, data_set, prefix=''):
        ...
        torch.cuda.empty_cache()
        # 批信息、损失累计
        batch_num, loss_sum = len(data_set), 0
        cuda = self.device.type != 'cpu'
        # 启动进度条, 调用 data_process 进行数据预处理
        pbar = tqdm(enumerate(data_set), total=batch_num)
        if not self.training: pbar.set_description(self._m_title)
        for i, batch in pbar:
            if self.training:
                with amp.autocast(enabled=cuda):
                    loss = self.loss(*batch)
                isfinite = torch.isfinite(loss)
                loss_sum += loss.item() if isfinite else loss_sum / (i + 1e-6)
                # loss 反向传播梯度，并迭代
            ...
            # 输出平均损失
            if self.training:
                cuda_mem = torch.cuda.memory_reserved() / 1e9
                avg_loss = loss_sum / (i + 1)
                pbar.set_description(self._l_format % (prefix, cuda_mem, avg_loss,
                                                       self._optim.param_groups[0]['lr'], not isfinite))
        torch.cuda.empty_cache()
        if self.training: yield loss_sum / batch_num
